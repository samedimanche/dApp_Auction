...
function AuctionHistory({ account, auctions }) {
  ...
  const currentTime = Math.floor(Date.now() / 1000);
  const endedAuctions = auctions.filter((auction) => {
    const startTime = auction.startTime?.toNumber ? auction.startTime.toNumber() : Number(auction.startTime);
    const duration = auction.duration?.toNumber ? auction.duration.toNumber() : Number(auction.duration);
            ...
            <div key={index} style={{ border: '1px solid #ccc', padding: '10px', marginBottom: '10px' }}>
              ...
              {expandedAuctionId === index && (
                <div>
                  ...
                  <p>Time Step: {auction.timeStep?.toNumber ? auction.timeStep.toNumber() : Number(auction.timeStep)} seconds</p>
